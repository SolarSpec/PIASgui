...
%             opts{1,end+1} = 'Cancel';
        ...
        if contains(name,chanFinal)
            ...
            app.NormKinPlot.YLabel.String = "ΔOD or %Abs";
        else
            selectedChans = chanRaw;
            app.KinPlot.YLabel.String = "Voltage (V)";
            app.NormKinPlot.YLabel.String = "Voltage (V)";
        end
...
%         AbsChanIndex = find(strcmp(TDMScontent.chanNames{1,CH0GroupIndex},name));
        ...
        if isempty(AbsChanIndex)
            given = contains(ChannelIndices,selectedChans);
            NewOpts = ChannelIndices(given);
            
            AbsChanIndex = find(strcmp(ChannelIndices,NewOpts{1,1}));
